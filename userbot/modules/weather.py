...
@register(outgoing=True, pattern="^.cuaca(?: |$)(.*)")
async def get_weather(weather):
    ...
    await weather.edit(
        ...
        +
        f"**Temperatur Minimal.:** `{celsius(min_temp)}°C | {fahrenheit(min_temp)}°F`\n"
        +
        f"**Temeperatur Maksimal.:** `{celsius(max_temp)}°C | {fahrenheit(max_temp)}°F`\n"
...
CMD_HELP.update({
    "weather":
    "`.cuaca` <kota> or `.cuaca` <kota>, <nama negara/kode>\
